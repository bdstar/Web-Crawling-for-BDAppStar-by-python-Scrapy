...
class MainSpider(scrapy.Spider):
    ...
    def phones(self, response):
        bname = response.css('h1.title::text').get()
        #bimage = response.css('div.area img').attrib['src']
        ...
        for phones in response.css('div.product-thumb'):
            ...
            yield scrapy.Request(url=link, callback=self.posts, meta={'pimage': pimage, 'pname': pname, 'bname': bname})
